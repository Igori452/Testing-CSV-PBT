...
void dumpGeneratedSamples(const std::string& filename, int count) {
    ...
    out << "=== CSV SAMPLES GENERATED BY RAPIDCHECK ===\n";
    out << "===========================================\n\n";
    
    int successCount = 0;
    std::unordered_set<std::string> uniqueCSVs;
    bool shouldStop = false;
    
    rc::check("Dump unique samples", [&]() {
        if (shouldStop) return true;
        
        for (int i = 0; i < count * 2 && !shouldStop; ++i) {
            auto table = *rc::gen::csvTable();
            auto csv = serializeTable(table);
            
            if (uniqueCSVs.find(csv) != uniqueCSVs.end()) {
                continue;
            }
            
            uniqueCSVs.insert(csv);
            successCount++;
            
            out << "--- Sample " << successCount << " ---\n";
            out << "Rows: " << table.size() << "\n";
            out << "CSV:\n" << csv << "\n\n";
            out.flush();
    ...
    out.close();
    std::cout << "✅ Saved " << successCount << " unique CSV samples to " << filename << "\n";
}
